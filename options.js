...
const els = {};
ids.concat(['soundKeys', 'soundUi', 'toastKey', 'toastIcon', 'toastText', 'soundVolume', 'soundScheme', 'toastPos', 'lang', 'likeConfirm', 'tilt', 'swapAnim', 'hoverRing', 'progressStyle', 'volumeStyle', 'progressWhite', 'progressOrange', 'volumeWhite', 'volumeOrange']).forEach((id) => { els[id] = document.getElementById(id); });
...
function applyOff(a) {
  const isOff = !!keyOffState[a];
  offBtns[a].classList.toggle('off', isOff);
  offBtns[a].closest('.row').classList.toggle('off', isOff);
