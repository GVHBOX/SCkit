(function () {
  ...
  if (rt && rt.onMessage) {
    rt.onMessage.addListener((msg, _sender, sendResponse) => {
      try {
        ...
        else if (msg.action === 'cmd') { applyCmd(msg); sendResponse(getState()); }
      } catch (e) {  }
    });
  }
  ...
  function ensureToastStyle() {
    if (document.getElementById('sckit-style')) return;
    const st = document.createElement('style');
    st.id = 'sckit-style';
    st.textContent =
      '#sckit-toast{position:fixed;bottom:84px;z-index:2147483647;width:max-content;max-width:calc(100vw - 32px);' +
      'display:flex;flex-direction:column;align-items:center;gap:5px;background:#17181c;border:2px solid #000;' +
      'border-radius:14px;box-shadow:0 4px 0 rgba(0,0,0,.45);padding:11px 15px 9px;' +
      'font:700 12.5px/1.3 system-ui,sans-serif;color:#e8eaed;pointer-events:none;' +
      ...
      'min-width:42px;height:42px;padding:0 9px;background:#ff5500;border:3px solid #000;' +
      'border-radius:10px;box-shadow:0 4px 0 #000;font:800 19px/1 Consolas,monospace;color:#fff;}' +
      '#sckit-toast .sckit-icon{font:400 17px/1 system-ui,"Segoe UI Emoji",sans-serif;}' +
  ...
  const TOAST_ICONS = { next: '⏭', prev: '⏮', ff: '⏩', rw: '⏪', like: '❤️', unlike: '💔', unlike_arm: '⚠️' };

  function toast(msg, key, icon, forceText) {
    ensureToastStyle();
    let t = document.getElementById('sckit-toast');
    if (!t) {
    ...
    const pos = cfg.toastPos === 'left' ? 'pL' : cfg.toastPos === 'center' ? 'pC' : 'pR';
    t.classList.remove('pL', 'pC', 'pR');
    t.classList.add(pos);
    const ky = t.querySelector('.sckit-key');
    ky.textContent = key || '';
    ky.style.display = (cfg.toastKey && key) ? '' : 'none';
    const ic = t.querySelector('.sckit-icon');
    ic.textContent = icon || '';
    ic.style.display = (cfg.toastIcon && icon) ? '' : 'none';
    const tx = t.querySelector('.sckit-txt');
    tx.textContent = msg;
    tx.style.display = (cfg.toastText || forceText) ? '' : 'none';
